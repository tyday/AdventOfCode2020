    ...
    with open(fileName, 'r') as f:
        ...
        for y in range(len(data)):
            for x in range(len(data[y])):
                if data[y][x] == '#':
                    location_dict[(x,y,0)] = {'status':True}
...
def get_neighbors(location):
    ...
    for x in range(-1,2):
        for y in range(-1,2):
            for z in range(-1,2):
                neighbor = (location[0]+x, location[1] + y, location[2] +z)
...
def print_grid(activeLocations):
    ...
    for zz in range(min(z),max(z)+1):
        print()
        print(f"{zz:3} " + "".join(str(y) for y in range(min(y),max(y)+1)))
        for yy in range(min(y),max(y)+1):
            row = f'{yy:2}  '
            for xx in range(min(x),max(x)+1):
                value = ''
                if (xx,yy,zz) in activeLocations:
                    value = '#'
                else:
                    value = '.'
                row += value
